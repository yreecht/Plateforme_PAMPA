...
#### lancement des fonctions génériques des extrapolations pour le bootstrap
source("C:/PAMPA/SCRIPTS WP3/ExtrapolationPourBootstrapFonctions180711.r")
...
# rajout d'une colonne semestre à la table freqtot
  FreqAnneeChoisie$semestre <- "sep_fev"
  FreqAnneeChoisie$semestre[which(is.element(FreqAnneeChoisie$mois,c(3,4,5,6,7,8)))] <- "mar_aout"
  assign("FreqAnneeChoisie",FreqAnneeChoisie,envir=.GlobalEnv)    
...
FreqEstimeeBatMoisAMP <- matrix(NA,ncol=length(unique(FreqAnneeChoisie$mois)),nrow=Nboot)               #  Nombre de bateaux extrapolé par mois
colnames(FreqEstimeeBatMoisAMP) <- unique(FreqAnneeChoisie$mois) 

FreqEstimeeBatTrimestreAMP <- matrix(NA,ncol=length(unique(FreqAnneeChoisie$trimestre)),nrow=Nboot)         #  Nombre de bateaux extrapolé par trimestre
colnames(FreqEstimeeBatTrimestreAMP) <- unique(FreqAnneeChoisie$trimestre) 

FreqEstimeeBatSemestreAMP <- matrix(NA,ncol=length(unique(FreqAnneeChoisie$semestre)),nrow=Nboot)         #  Nombre de bateaux extrapolé par semestre
colnames(FreqEstimeeBatSemestreAMP) <- unique(FreqAnneeChoisie$semestre) 

FreqEstimeeBatAnneeAMP <- matrix(NA,ncol=1,nrow=Nboot)             # Nombre de bateaux extrapolés à l'année sur l'AMP
colnames(FreqEstimeeBatAnneeAMP) <- anneeChoisie 

FreqEstimeeBatAnneeZone <- matrix(NA,ncol=length(unique(FreqAnneeChoisie$zone)),nrow=Nboot)            #  Nombre de bateaux extrapolé à l'année par zone
colnames(FreqEstimeeBatAnneeZone) <- unique(FreqAnneeChoisie$zone) 

FreqEstimeeBatAnneeGroupe <- matrix(NA,ncol=length(unique(FreqAnneeChoisie$groupe)),nrow=Nboot)           #  Nombre de bateaux extrapolé à l'année par groupe de zones
colnames(FreqEstimeeBatAnneeGroupe) <- unique(FreqAnneeChoisie$groupe) 

FreqEstimeeBatAnneeZonage <- matrix(NA,ncol=length(unique(FreqAnneeChoisie$statut)),nrow=Nboot)          #  Nombre de bateaux extrapolé à l'année par zonage PAMPA
colnames(FreqEstimeeBatAnneeZonage) <- unique(FreqAnneeChoisie$statut) 

########################## PAR ACTIVITE  (même format que pour toutes activités confondues
FreqEstimeeBatActMoisAMP <- FreqEstimeeBatMoisAMP               #  Nombre de bateaux extrapolé par activité par mois
FreqEstimeeBatActTrimestreAMP <- FreqEstimeeBatTrimestreAMP         #  Nombre de bateaux extrapolé par activité par trimestre
FreqEstimeeBatActSemestreAMP <- FreqEstimeeBatSemestreAMP         #  Nombre de bateaux extrapolé par activité par trimestre
FreqEstimeeBatActAnneeAMP <- FreqEstimeeBatAnneeAMP                                               # Nombre de bateaux extrapolés par activité à l'année sur l'AMP
FreqEstimeeBatActAnneeZone <- FreqEstimeeBatAnneeZone            #  Nombre de bateaux extrapolé par activité à l'année par zone
FreqEstimeeBatActAnneeGroupe <- FreqEstimeeBatAnneeGroupe           #  Nombre de bateaux extrapolé par activité à l'année par groupe de zones
FreqEstimeeBatActAnneeZonage <- FreqEstimeeBatAnneeZonage          #  Nombre de bateaux extrapolé par activité à l'année par zonage PAMPA

########################## PAR CATEGORIE D'ACTIVITES
FreqEstimeeBatTypActMoisAMP <- FreqEstimeeBatMoisAMP               #  Nombre de bateaux extrapolé par catégorie d'activité par mois
FreqEstimeeBatTypActTrimestreAMP <- FreqEstimeeBatTrimestreAMP         #  Nombre de bateaux extrapolé par catégorie d'activité par trimestre
FreqEstimeeBatTypActSemestreAMP <- FreqEstimeeBatSemestreAMP         #  Nombre de bateaux extrapolé par catégorie d'activité par trimestre
FreqEstimeeBatTypActAnneeAMP <- FreqEstimeeBatAnneeAMP                                               # Nombre de bateaux extrapolés par catégorie d'activité à l'année sur l'AMP
FreqEstimeeBatTypActAnneeZone <- FreqEstimeeBatAnneeZone            #  Nombre de bateaux extrapolé par catégorie d'activité à l'année par zone
FreqEstimeeBatTypActAnneeGroupe <- FreqEstimeeBatAnneeGroupe           #  Nombre de bateaux extrapolé par catégorie d'activité à l'année par groupe de zones
FreqEstimeeBatTypActAnneeZonage <- FreqEstimeeBatAnneeZonage          #  Nombre de bateaux extrapolé par catégorie d'activité à l'année par zonage PAMPA

# en nombre de personnes
########################## TOUTES ACTIVITES CONFONDUES
FreqEstimeePersMoisAMP <- FreqEstimeeBatMoisAMP               #  Nombre de personnes extrapolé par mois
FreqEstimeePersTrimestreAMP <- FreqEstimeeBatTrimestreAMP         #  Nombre de personnes extrapolé par trimestre
FreqEstimeePersSemestreAMP <- FreqEstimeeBatSemestreAMP         #  Nombre de personnes extrapolé par semestre
FreqEstimeePersAnneeAMP <- FreqEstimeeBatAnneeAMP           # Nombre de personnes extrapolés à l'année sur l'AMP
FreqEstimeePersAnneeZone <- FreqEstimeeBatAnneeZone            #  Nombre de personnes extrapolé à l'année par zone
FreqEstimeePersAnneeGroupe <- FreqEstimeeBatAnneeGroupe           #  Nombre de personnes extrapolé à l'année par groupe de zones
FreqEstimeePersAnneeZonage <- FreqEstimeeBatAnneeZonage          #  Nombre de personnes extrapolé à l'année par zonage PAMPA

########################## PAR ACTIVITE  (même format que pour toutes activités confondues
FreqEstimeePersActMoisAMP <- FreqEstimeeBatMoisAMP               #  Nombre de personnes extrapolé par activité par mois
FreqEstimeePersActTrimestreAMP <- FreqEstimeeBatTrimestreAMP         #  Nombre de personnes extrapolé par activité par trimestre
FreqEstimeePersActSemestreAMP <- FreqEstimeeBatSemestreAMP         #  Nombre de personnes extrapolé par activité par trimestre
FreqEstimeePersActAnneeAMP <- FreqEstimeeBatAnneeAMP               # Nombre de personnes extrapolés par activité à l'année sur l'AMP
FreqEstimeePersActAnneeZone <- FreqEstimeeBatAnneeZone            #  Nombre de personnes extrapolé par activité à l'année par zone
FreqEstimeePersActAnneeGroupe <- FreqEstimeeBatAnneeGroupe           #  Nombre de personnes extrapolé par activité à l'année par groupe de zones
FreqEstimeePersActAnneeZonage <- FreqEstimeeBatAnneeZonage          #  Nombre de personnes extrapolé par activité à l'année par zonage PAMPA

########################## PAR CATEGORIE D'ACTIVITES
FreqEstimeePersTypActMoisAMP <- FreqEstimeeBatMoisAMP               #  Nombre de personnes extrapolé par catégorie d'activité par mois
FreqEstimeePersTypActTrimestreAMP <- FreqEstimeeBatTrimestreAMP         #  Nombre de personnes extrapolé par catégorie d'activité par trimestre
FreqEstimeePersTypActSemestreAMP <- FreqEstimeeBatSemestreAMP         #  Nombre de personnes extrapolé par catégorie d'activité par trimestre
FreqEstimeePersTypActAnneeAMP <- FreqEstimeeBatAnneeAMP                                               # Nombre de bateaux extrapolés par catégorie d'activité à l'année sur l'AMP
FreqEstimeePersTypActAnneeZone <- FreqEstimeeBatAnneeZone            #  Nombre de personnes extrapolé par catégorie d'activité à l'année par zone
FreqEstimeePersTypActAnneeGroupe <- FreqEstimeeBatAnneeGroupe           #  Nombre de personnes extrapolé par catégorie d'activité à l'année par groupe de zones
FreqEstimeePersTypActAnneeZonage <- FreqEstimeeBatAnneeZonage          #  Nombre de personnes extrapolé par catégorie d'activité à l'année par zonage PAMPA
