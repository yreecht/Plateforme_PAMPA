################################################################################
# Nom              : import données WP3.r
# Type              : Programme
# Objet             : Programme d'importation des données à partir des fichiers 
#                     texte enregistrés dans le dossier C:/PAMPA/DONNEES WP3.
# Input             : TXT et SHP
# Output            : Data
# Auteur            : Elodie Gamp
# R version         : 2.8.1
...
# message d'information et de rappel pour l'importation des données dans R
  infoLoading.f(msg=, "Les tables sont importées automatiquement!            
                    Vérifiez que : 
                        - les noms des fichiers sont bien ceux précisés dans le guide d'utilisation des scripts WP3
                        - tous les fichiers sont dans le dossier 'C:/PAMPA/DONNEES WP3'.", 
                icon="info")
...
### importation du référentiel espèces
  ...
  refEspeces=read.table(file="C:/PAMPA/DONNEES WP3/refEspeces.txt", sep="\t", dec=".", header=T)
  # 125 colonnes
...
### importation du référentiel spatial de l'AMP
  ...
  refSpatial = read.table(file="C:/PAMPA/DONNEES WP3/refSpatial.txt", header=T, sep="\t", dec=".")
  # 15 colonnes
  #Vérification du nombre de colonnes:
  ...
  refEngin = read.table(file="C:/PAMPA/DONNEES WP3/refEngin.txt", header=T, sep="\t", dec=".")
  # 15 colonnes
  #Vérification du nombre de colonnes:
    if (dim(refEngin)[2] != 9){
...
###### import des données de fréquentation  
  ...
  freqtot=read.table(file="C:/PAMPA/DONNEES WP3/FREQUENTATION_export.txt", sep="\t", dec=".", header=T)
  # 35 colonnes
  #Vérification du nombre de colonnes:
  if (dim(freqtot)[2] != 35){
...
#    freqtot$moisAn<-paste(freqtot$annee, "_", freqtot$mois, sep="")
    ...
    write.csv(freqTotPeriod, file="C:/PAMPA/resultats script R/metriquesWP3/recensement fréquentation total.csv", row.names = F)
  }
...
###### import des données des questionnaires de pêche récréative
  ...
  peche=read.table(file="C:/PAMPA/DONNEES WP3/ENQUETE PECHE_export.txt", sep="\t", dec=".", header=T)
  # 115 colonnes
  #Vérification du nombre de colonnes:
    if (dim(peche)[2] != 116){
...
###### import des données des captures du jour
  ...
  captures=read.table(file="C:/PAMPA/DONNEES WP3/CAPTURES_export.txt", sep="\t", dec=".", header=T)
  #34 colonnes
  #Vérification du nombre de colonnes:
...
# tableau du nombre total de captures
  ...
  write.csv(capturesTot, file="C:/PAMPA/resultats script R/metriquesWP3/capture totale.csv", row.names = T)
}      
...
###### import des données des captures annuelles
  ...
  capturesAn=read.table(file="C:/PAMPA/DONNEES WP3/CAPTURES ANNUELLES_export.txt", sep="\t", dec=".", header=T)
  # 29colonnes
  #Vérification du nombre de colonnes:
...
###### import des données des questionnaires de plaisance
  ...
  plaisance=read.table(file="C:/PAMPA/DONNEES WP3/ENQUETE PLAISANCE_export.txt", sep="\t", dec=".", header=T)
  # 77colonnes
  #Vérification du nombre de colonnes:
...
###### import des données des questionnaires plongée
  ...
  plongee=read.table(file="C:/PAMPA/DONNEES WP3/ENQUETE PLONGEE_export.txt", sep="\t", dec=".", header=T)
  # 70 colonnes
  #Vérification du nombre de colonnes:
  if (dim(plongee)[2] != 70){
...
###### import des données des questionnaires d'excursion
  ...
  excursion=read.table(file="C:/PAMPA/DONNEES WP3/ENQUETES EXCURSION-PMT_export.txt", sep="\t", dec=".", header=T)
  # 70 colonnes
  #Vérification du nombre de colonnes:
...
#  carte=read.shape("C:/PAMPA/DONNEES WP3/carte/cartefile")     # à revoir avec les nouvelles versions de R
...
source("C:/PAMPA/SCRIPTS WP3/FonctionsCalculsObligatoires.r")
