...
MoyenneStratifieeTypeJ.f <- function(tab, MoyFreqJour , InfoJour , niveauTemporel)
{
    ...
    ### Somme par niveauTemporel choisi
       if (length(unique(tab[,niveauTemporel]))!=1) {
        ...
        Indx <- unique(calendrierGeneral[order(calendrierGeneral$mois) ,
...
BoxplotFreqTypeJ.f <- function(tab, variable , niveauSpatial , niveauTemporel, MoyStrat, periode)
{
    ...
    # boxplot appliqué sur chaque niveauTemporel
      if (length(unique(tab[ , niveauTemporel])) != 1) {
          sapply(seq(1 : dim(freqZone)[4]) ,                                   # premier sapply sur le type de jour
                FUN = function(j) {sapply(seq(1:dim(freqZone)[3]),           # deuxième sapply sur le niveau temporel
                                          FUN = function(x , j){boxplot(freqZone[, , x , j] ,
      ...
      mtext(paste(periode, "fréquentation en" , variable , "par type de jours, par" , niveauTemporel ,"et par" , niveauSpatial) ,
            line=2 , outer=TRUE , cex = 3)
      savePlot(filename = paste("C:/PAMPA/resultats script R/metriquesWP3/pression/boxplotfréquentation en" ,
                                variable , "par type de jours, par" , niveauTemporel ,"et par" , niveauSpatial, periode), type =c("png"))
    ...
    sink(paste("C:/PAMPA/resultats script R/metriquesWP3/pression/frequentation" , variable , "par" , niveauSpatial , "et" , niveauTemporel , periode, ".txt"))
    ...
    # boxplot appliqué sur chaque niveauTemporel
      if (length(unique(tab[ , niveauTemporel])) != 1) {
          sapply(seq(1 : dim(freqZone)[3]) ,
                 FUN = function(x){boxplot(freqZone[, , x] ,
      ...
      mtext(paste(periode, "fréquentation en" , variable , "par" , niveauTemporel ,"et par" , niveauSpatial) ,
            line = 2 , outer = TRUE , cex = 3)
      savePlot(filename = paste("C:/PAMPA/resultats script R/metriquesWP3/pression/boxplotfréquentation en" ,
                                variable , "par" , niveauTemporel ,"et par" , niveauSpatial, periode), type =c("png"))
...
BoxplotFreqTypeJFacteur.f <- function(tab, variable , niveauSpatial , niveauTemporel, MoyStrat, periode, facteurSep, modalites)
{
  ...
  #### manip de transfo de fréquentation (FreqJour) pour prise en compte des 0 lors d'une sélection
   ...
   idx <- with(tab,
              tapply(tab[ , variable],
                        as.list(tab[, c("numSortie", niveauSpatial,                     # éléments nommés pour avoir les noms de dimensions dans le tableau
                                        niveauTemporel, "typeJsimp", facteurSep )]),                     # Les mêmes moins le champ de sélection
                     function(x)ifelse(length(x), TRUE, FALSE)))
  ...
  # Tableau de fréquentation par facteurSep, corrigé pour
   ## tenir compte des activités non observées sur sorties effectives :
    ...
    # boxplot appliqué sur chaque niveauTemporel
      ...
      mtext(paste(periode, "fréquentation en" , variable , "par type de jours, par" , niveauTemporel ,"et par" , niveauSpatial, facteurSep, "=", modalites) ,
            line=2 , outer=TRUE , cex = 3)
      savePlot(filename = paste("C:/PAMPA/resultats script R/metriquesWP3/pression/boxplotfréquentation en" ,
                                variable , "par type de jours, par" , niveauTemporel ,"et par" , niveauSpatial, periode, facteurSep, "=", modalites), type =c("png"))
    ...
    TabStatDescrip <- list("nbSorties" = nbSortie , "moyenneStratifieeFreq" = MoyStrat ,
                           "miniFreq" = miniFreq , "maxiFreq" = maxiFreq ,
                           "medianeFreq" = medianeFreq)
    sink(paste("C:/PAMPA/resultats script R/metriquesWP3/pression/frequentation" , variable , "par" , niveauSpatial , "et" , niveauTemporel , periode, facteurSep, "=", modalites, ".txt"))
    print(TabStatDescrip)
    sink()
}
...
BoxplotFreqTousJFacteur.f <- function(tab, variable , niveauSpatial , niveauTemporel, MoyStrat, periode, facteurSep, modalites)
{
  ...
  #### manip de transfo de fréquentation (FreqJour) pour prise en compte des 0 lors d'une sélection
   ...
   idx <- with(tab,
              tapply(tab[ , variable],
                        as.list(tab[, c("numSortie", niveauSpatial,                     # éléments nommés pour avoir les noms de dimensions dans le tableau
                                        niveauTemporel, facteurSep )]),                     # Les mêmes moins le champ de sélection
                     function(x)ifelse(length(x), TRUE, FALSE)))
  ...
  # Tableau de fréquentation par facteurSep, corrigé pour
   ## tenir compte des activités non observées sur sorties effectives :
    ...
    # boxplot appliqué sur chaque niveauTemporel
      ...
      mtext(paste(periode, "fréquentation en" , variable , "par" , niveauTemporel ,"et par" , niveauSpatial, facteurSep, "=", modalites) ,
            line = 2 , outer = TRUE , cex = 3)
      savePlot(filename = paste("C:/PAMPA/resultats script R/metriquesWP3/pression/boxplotfréquentation en" ,
                                variable , "par" , niveauTemporel ,"et par" , niveauSpatial, periode, facteurSep, "=", modalites), type =c("png"))
