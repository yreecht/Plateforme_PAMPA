################################################################################
# Nom               : BootstrapEnquetes.r
# Type              : Programme
# Objet             : Fonctions bootstrap des métriques d'enquêtes à partir 
#                     des questionnaires des différents usagers. 
#                     Ces fonctions seront appelées dans l'interface relative aux 
#                     calculs des données d'enquêtes.
...
BootstrapOpinionQuanti.f <- function(tableau, facteur, metrique)
{
  ...
  ## déclaration du tableau de stockage pour les résultats bootstrap
    questBoot <- array(NA,c(1,lengthnna.f(unique(tableau [ , facteur ])),Nboot), 
                       dimnames=list("valeur",na.exclude(unique(tableau [ , facteur ])),c(1:Nboot)))
  ...
  ICsupboot <- apply(questBoot,c(1,2),quantile,probs=0.975,na.rm=T)
  estim <- array(c(ICinfboot,moyboot,ICsupboot),
                 c(1,lengthnna.f(unique(tableau [ , facteur ])),3))
  dimnames(estim) <- list (rownames(moyboot),colnames(moyboot),c("ICInf","Moy","ICSup"))
  return(estim)
