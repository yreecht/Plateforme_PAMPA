...
arbre1.f <- function ()
{
    ...
    OnOK <- function()
    {
        ...
        if (length(arbre1$cptable[,"CP"])<2)
        {
            ...
            summary(arbre1rpart)
            print(arbre1rpart)
            plot(arbre1rpart,branch=0.2,compress=T, margin=0.1,main=paste("Arbre de régression (RPART) pour la métrique: ",me,"\n et le facteur de variabilité: ",fact1arbre[1]))
            text(arbre1rpart,use.n=T,pretty=0,all=T)
            ## plotcp(arbre1rpart)
            capture.output(print(arbre1),print(summary(arbre1)), file=paste(nameWorkspace,"/FichiersSortie/arbre de régression-MVPART_",me,fact1arbre,".txt",sep=""))
            capture.output(print(arbre1rpart),print(summary(arbre1rpart)), file=paste(nameWorkspace,"/FichiersSortie/arbre de régression-RPART_",me,fact1arbre[1],".txt",sep=""))
        }
    }
...
arbre2.f <- function ()
{
    ...
    OnOK <- function()
    {
        ...
        x11(width=50,height=30,pointsize=10)
        arbre2 <- mvpart(data.matrix(unitbis[,me]) ~ unitbis[,fact1arbre[1]]+unitbis[,fact1arbre[2]], unitbis, xv="1se")  #pca=T
        if (length(arbre2$cptable[,"CP"])<2)
        {
            ...
            summary(arbre2)
            print(arbre2)
            arbre2rpart <- rpart(data.matrix(unitbis[,me]) ~ unitbis[,fact1arbre[1]]+unitbis[,fact1arbre[2]], unitbis)
            x11(width=50,height=30,pointsize=10)
            summary(arbre2rpart)
            print(arbre2rpart)
            plot(arbre2rpart, branch=0.2, compress=T, margin=0.1,
                 main=paste("Arbre de régression (RPART) pour la metrique: ", me,
                 "\n et les facteurs de variabilité: ", fact1arbre[1], "et", fact1arbre[2]))
            text(arbre2rpart,use.n=T,pretty=0,all=T)
            ## plotcp(arbre2rpart)
...
arbre3.f <- function ()
{
    ...
    OnOK <- function()
    {
        ...
        tkmessageBox(message=paste("Les facteurs choisis sont renseignés pour ",nrow(unitbis)," données.",sep=""))
        x11(width=50,height=30,pointsize=10)
        arbre3 <- mvpart(data.matrix(unitbis[,me]) ~ unitbis[,fact1arbre[1]]+unitbis[,fact1arbre[2]]+unitbis[,fact1arbre[3]], unitbis, xv="1se")
        if (length(arbre3$cptable[,"CP"])<2)
        {
            ...
            summary(arbre3)
            arbre3rpart <- rpart(data.matrix(unitbis[,me]) ~ unitbis[,fact1arbre[1]]+unitbis[,fact1arbre[2]]+unitbis[,fact1arbre[3]], unitbis)
            x11(width=50,height=30,pointsize=10)
            summary(arbre3rpart)
            print(arbre3rpart)
            plot(arbre3rpart,branch=0.2,compress=T, margin=0.1,main=paste("Arbre de régression (RPART) pour la métrique: ",me,"\n et les facteurs de variabilité: ",fact1arbre[1],fact1arbre[2],"et",fact1arbre[3]))
            text(arbre3rpart,use.n=T,pretty=0,all=T)
            ## plotcp(arbre2rpart)
            capture.output(print(arbre3),print(summary(arbre3)), file=paste(nameWorkspace,"/FichiersSortie/arbre de régression-MVPART_",me,fact1arbre[1],fact1arbre[2],fact1arbre[3],".txt",sep=""))
            capture.output(print(arbre3rpart),print(summary(arbre3rpart)), file=paste(nameWorkspace,"/FichiersSortie/arbre de régression-RPART_",me,fact1arbre[1],fact1arbre[2],fact1arbre[3],".txt",sep=""))
        }
    }
    OK.but <-tkbutton(bb,text="OK",command=OnOK)
    tkgrid(OK.but)
