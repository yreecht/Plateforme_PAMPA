...
import.donnees.f <- function()
{
    ...
    sitesPAMPA <- c("Banyuls", "Bonifacio", "Cap Roux", "Côte Bleue", "Mayotte",
                    "Nouvelle-Caledonie", "Reunion", "Saint-Martin")
    ...
    names(unitobs) <- c("AMP", "unite_observation", "type", "site", "station", "caracteristique_1", "caracteristique_2",
                        "fraction_echantillonnee", "jour", "mois", "an", "heure", "nebulosite", "direction_vent",
                        "force_vent", "etat_mer", "courant", "maree", "phase_lunaire", "latitude", "longitude",
                        "statut_protection", "avant_apres", "biotope", "biotope_2", "habitat1", "habitat2", "habitat3",
                        "visibilite", "prof_min", "prof_max", "DimObs1", "DimObs2", "nb_plong", "plongeur")

    unitobs$caracteristique_1[is.na(unitobs$caracteristique_1)] <- "na"

    ## Calcul de la durée pour la pêche récréative (d'après heures de début et fin) :
    if(unique(unitobs$caracteristique_1)=="PecRec")
    {
        ...
        unitobs <- unitobs[, c("AMP", "unite_observation", "type", "site", "station", "caracteristique_1",
                               "caracteristique_2", "fraction_echantillonnee", "jour", "mois", "an", "heure",
                               "nebulosite", "direction_vent", "force_vent", "etat_mer", "courant", "maree",
                               "phase_lunaire", "latitude", "longitude", "statut_protection", "avant_apres", "biotope",
                               "biotope_2", "habitat1", "habitat2", "habitat3", "visibilite", "prof_min", "prof_max",
                               "DimObs1", "DimObs2", "nb_plong", "plongeur")]
    }
    ...
    {
        names(obs) <- c("unite_observation", "secteur", "code_espece", "sexe", "taille", "classe_taille", "poids",
                        "nombre", "dmin", "dmax")
    }else{
        names(obs) <- c("unite_observation", "rotation", "secteur", "code_espece", "sexe", "taille", "classe_taille",
    ...
    ## la proportion de D2-D1 si on a une observation avec D1 <5m et D2 >5m
    ## (sinon sous-estimation des metriques comptage) :
    ...
    ## suppression des observations dont le nombre d'individus est à zero : [!!!] nécessaire ?
    if (dim(subset(obs, nombre == 0))[1] > 0)
    {
        obs <- subset(obs, nombre != 0)
        ## tkmessageBox(message="Des observations dont le nombre d'individus est à 0 ont ete supprimees.",
        ##              icon="warning", type="ok")
    }
    ...
    if (all(!is.na(unitobs$site)))      # (NA %in% unique(unitobs$site) == FALSE) # [!!!]
    {
        PlanEchantillonnage <- with(unitobs,
                                    table(an, caracteristique_1, biotope, statut_protection, exclude = NA))
    }else{
        if (all(!is.na(unitobs$biotope))) # (NA %in% unique(unitobs$biotope) == FALSE) # [!!!]
        {
    ...
    if (unique(unitobs$type) != "SVR")
    {
        ...
        contingenceSVRt <- tapply(obs$nombre,
                                  list(obs$unite_observation, obs$rotation, obs$code_espece),
                                  mean, na.rm=TRUE)
        ...
        contingenceSVR$abondance <- as.vector(contingenceSVRt)                                    #
                                                                                                  # certainement
        contingenceSVR$unitobs <- rep(dimnames(contingenceSVRt)[[1]],                             # moyen de faire
                                      times = dim(contingenceSVRt)[2] * dim(contingenceSVRt)[3])  # plus simple
        contingenceSVR$rotation <- rep(dimnames(contingenceSVRt)[[2]],                            # que tout ça !
                                       each = dim(contingenceSVRt)[1],                            # [!!!]
                                       times = dim(contingenceSVRt)[3])                           # [yr: 18/11/2010]
        ...
        contingence <- tapply(contingenceSVR$abondance,                                           #
                              list(contingenceSVR$unitobs, contingenceSVR$code_espece),           #
                              sum, na.rm=TRUE)                                                    #
    }
    ...
    {
        tkmessageBox(message="ATTENTION, differents types d'observations ne peuvent être analyses ensemble!",
                     icon="warning", type="ok")
    }else{
     ...
     ont été créés", icon="warning", type="ok") # spell checking !!! [yr: 27/07/2010] [OK]
    }
