    ...
    if (!getOption("P.graphPDF")) # sorties graphiques à l'écran ou PNG.
    {
        if (isTRUE(getOption("P.graphPNG")))
        {
            if (noGraph == 1 || ! getOption("P.plusieursGraphPage"))
            {
                ...
                {
                    tiff(pngFileName,
                         width=ifelse(large, 120, 90) * 4.2 * 15,
                         height=ifelse(large, 75, 55) * 4.2 * 15,
                         pointsize=14, res=300, compression="lzw")
                    par(mfrow=c(getOption("P.nrowGraph"), getOption("P.ncolGraph")))
                }else{
                    tiff(pngFileName,
                         width=ifelse(large, 100,
                                      ifelse(isTRUE(getOption("P.graphPaper")), 50, 75)) * 4.2 * 15,
                         height=ifelse(large, 55,
                                       ifelse(isTRUE(getOption("P.graphPaper")), 30, 40)) * 4.2 * 15,
                         pointsize=14,
