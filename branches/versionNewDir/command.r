RestaurerDonnees.f = function() {
  ...
  if (Jeuxdonnescoupe==1)
  {
   ...
   if (unique(unitobs$type) != "LIT") {  #car pas de classes de tailles avec les recouvrements
  ...
  tkmessageBox(message=paste("Jeu de données restauré \n",dim(obs)[1],"enregistrements dans la table observation"))
...
## Objet  : exécution de la sélection par phylum et écrasement des données dans "obs"
################################################################################
...
## Objet  : exécution de la sélection par classe et écrasement des données dans "obs"
################################################################################
...
## Objet  : exécution de la sélection par cathégorie benthique et écrasement des données dans "obs"
################################################################################
...
## Objet  : exécution de la sélection par statut et écrasement des données dans "obs"
################################################################################
...
taillemoyenne.f = function() {          # [!!!] [inc] Fonction fausse / pas finie ! [yr: 13/08/2010]
...
GraphMetriqueParFacteurEspece.f  = function () {        # a mettre en fin de menu graphbenthos (interface.r), tester puis généraliser dans Graph...
...
GraphMetriqueParFacteurUnitobs.f  = function () {       # a mettre en fin de menu graphbenthos (interface.r)
  print("fonction GraphMetriqueParFacteurEspece.f activée")
  matable="listespunit"
  choixchamptable.f(matable)
  metrique=champtrouve
